...
export function playFancyModelInput(entity: Entity, values: FancyModelAnimationInput): void {
  const assignments = Object.entries(values).map(([name, value]) => (
    `v.${name}=${value};`
  ));
  // Replace one persistent input controller with the latest complete snapshot.
  // Pose targets use this same interface when the body is driven by physics.
