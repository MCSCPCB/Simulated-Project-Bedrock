// Broad-phase indexes for sub-level entity collision: a world-space cell index
// over per-tick actor snapshots and a body-local cell index over response
// boxes, plus the swept bounds both are queried with. Migrated from TreePhysics
// src/physics/collision/ContraptionColliderIndex.ts.
import type { Vector3 } from "@minecraft/server";
import { add, scale } from "../../util/SableVector3Utils.js";
import { rotateVectorByEulerDegreesYzx } from "../../api/math/RotationContinuity.js";
import { DEFAULT_OBB_SETTINGS, type ObbEntitySnapshot } from "./obb/ObbTypes.js";
import type { CollisionBoxState, WorldBounds } from "./SubLevelEntityCollision.js";
...
export class SubLevelCollisionSnapshotIndex {
  ...
  query(subLevelBounds: WorldBounds, bodyVelocity: Vector3): ObbEntitySnapshot[] {
    const bounds = createMovingSubLevelBounds(subLevelBounds, bodyVelocity);
...
function createMovingSubLevelBounds(bounds: WorldBounds, velocity: Vector3): WorldBounds {
...
export function createSubLevelRotationSweepBounds(previousFrame: { origin: Vector3 }, frame: { origin: Vector3 }, localBounds: WorldBounds): WorldBounds {
