// Box-collider geometry for the sub-level entity collision proxy: descriptor
// extraction from a rigid-body collider, body-frame construction, OBB pose
// derivation with continuous Euler angles, and the swept AABB-vs-OBB hit time.
// Migrated from TreePhysics src/physics/collision/ContraptionColliderMath.ts.
import type { AABB, Vector3 } from "@minecraft/server";
import type { RigidBodyHandle } from "../../api/physics/handle/RigidBodyHandle.js";
import type { ObbTransform } from "./obb/ObbTypes.js";
...
import { EPSILON_1E6, add, clamp, dot, scale, subtract } from "../../util/SableVector3Utils.js";

export interface SubLevelCollisionBoxDescriptor {
...
export function getSubLevelCollisionBoxDescriptors(
  collider: PhysicsBodyCollider
): SubLevelCollisionBoxDescriptor[] {
  ...
  if (collider.type !== "compound") {
    throw new TypeError(
      `Sub-level collision requires a box-based collider, received ${collider.type}.`
    );
  }

  const boxes: SubLevelCollisionBoxDescriptor[] = [];
  for (let index = 0; index < collider.children.length; index++) {
    ...
    if (child.collider.type !== "box") {
      throw new TypeError(
        `Sub-level collision child ${index} must be a box, received ${child.collider.type}.`
      );
    }
    boxes.push({
...
export function createBodyFrame(body: Pick<RigidBodyHandle, "localPointToWorld">): BodyFrame {
...
export function getSubLevelCollisionBoxTransformFromFrame(
  frame: BodyFrame,
  box: SubLevelCollisionBoxDescriptor,
...
export function getSweptAabbObbHitTime(
  ...
  actorExtent: Vector3,
  box: { readonly transform: ObbTransform; readonly descriptor: SubLevelCollisionBoxDescriptor }
): number | undefined {
...
export function collisionBoxKey(box: SubLevelCollisionBoxDescriptor): string {
  return [
...
function getBoxSize(collider: Readonly<{ halfExtents?: Vector3; size?: Vector3 }>): Vector3 {
  ...
  if (!isFiniteVector(size) || size.x <= 0 || size.y <= 0 || size.z <= 0) {
    throw new RangeError("Sub-level collision box size must contain positive finite components.");
  }
  return { ...size };
}
...
function normalize(value: Vector3): Vector3 {
  ...
  if (!Number.isFinite(length) || length <= ANGLE_EPSILON) {
    throw new RangeError("Sub-level collision orientation axes must be finite and non-zero.");
  }
