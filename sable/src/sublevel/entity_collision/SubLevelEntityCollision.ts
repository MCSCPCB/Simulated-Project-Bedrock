// Owns the native collision surface of one sub-level: keeps a solid OBB per
// response box, selects the boxes near each actor, and applies the combined
// carrier-inheritance and slip-friction impulse. Migrated from TreePhysics
// src/physics/collision/ContraptionCollider.ts.
import {
  system,
  world,
  type AABB,
...
import { SolidObb } from "./obb/SolidObb.js";
import { DEFAULT_OBB_SUPPORT_PRESET_CATALOG } from "./obb/ObbPresets.js";
import {
  ...
  type ObbTransform
} from "./obb/ObbTypes.js";
import { calculateSurfaceMotionImpulse } from "./obb/internal/Motion.js";
import type { PhysicsBodyCollider } from "../../api/physics/PhysicsTypes.js";
import {
  EPSILON_1E6,
...
import {
  ...
  SubLevelCollisionSnapshotIndex,
  createSubLevelRotationSweepBounds as createIndexedSubLevelRotationSweepBounds,
  getDescriptorBounds as getIndexedDescriptorBounds
} from "./SubLevelEntityCollisionIndex.js";
import {
  assertAabb,
  collisionBoxKey,
  createBodyFrame,
  getSweptAabbObbHitTime,
  getSubLevelCollisionBoxDescriptors,
  getSubLevelCollisionBoxTransformFromFrame,
  isFiniteVector,
  lerp,
  type BodyFrame,
  type SubLevelCollisionBoxDescriptor,
  worldToLocal
} from "./SubLevelEntityCollisionMath.js";

export const BLOCK_COLLIDER_ENTITY_TYPE_ID = "sable:block_collider";
...
export type SubLevelSurfaceContactCallback = (
  entity: Entity,
  location: Vector3,
...
export interface CollisionBoxState {
  readonly descriptor: SubLevelCollisionBoxDescriptor;
  readonly key: string;
...
/**
 * Owns the native Minecraft collision surface for one sub-level.
 ...
 */
export class SubLevelEntityCollision {
  ...
  constructor(body: RigidBodyHandle, collider: PhysicsBodyCollider, enabled = true) {
    this.#body = body;
  ...
  #replaceCollider(collider: PhysicsBodyCollider): void {
    ...
    const descriptors = getSubLevelCollisionBoxDescriptors(collider);
    ...
    try {
      for (const descriptor of descriptors) {
        ...
        const transform = getSubLevelCollisionBoxTransformFromFrame(frame, descriptor);
        const state = {
          descriptor,
          key,
          ...
          obb: new SolidObb({
            ...
            // Sub-level creation never performs a separate native actor query.
            entitySnapshots: [],
            settings: {
  ...
  sync(
    ...
    surfaceContactCallback?: SubLevelSurfaceContactCallback
  ): void {
    ...
    if (!localBounds) throw new Error("Sub-level collider has no local bounds.");
    const snapshots = snapshotIndex.query(
      createIndexedSubLevelRotationSweepBounds(previousFrame, frame, localBounds),
      this.#body.velocity
    );
  ...
  #assertProxyLedgerMatchesActiveBoxes(): void {
    ...
    if (expectedCount !== this.#managedProxyIds.size) {
      throw new Error("Sub-level collider ledger does not match active OBB support counts.");
    }
  }
}
...
/** Samples each player once for reuse by every sub-level and OBB this tick. */
export function createSubLevelCollisionEntitySnapshots(
...
export function removeStaleSubLevelColliders(): void {
...
/** Removes a loaded collision proxy unless a live sub-level currently owns it. */
