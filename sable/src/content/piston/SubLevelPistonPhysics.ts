// Turns vanilla piston activation into queued sub-level forces: the pushed
// cells are scanned along the piston axis, the moved blocks are reported for
// caller bookkeeping and cache invalidation, and the front faces are queued on
// the force queue. Migrated from TreePhysics src/events/contraption/Piston.ts.
import {
  system,
  world,
  type Block,
...
import type { SubLevelForceQueue } from "../../api/physics/force/SubLevelForceQueue.js";
import {
  PISTON_PLAYER_IMPULSE_MULTIPLIER,
  SLIME_PISTON_PLAYER_IMPULSE_MULTIPLIER
} from "../../api/physics/force/SubLevelForceMath.js";
...
export function installSubLevelPistonPhysics(
  effects: SubLevelForceQueue,
  ...
  world.afterEvents.pistonActivate.subscribe(event => {
    ...
    // block handles used for caller bookkeeping and material metadata.
