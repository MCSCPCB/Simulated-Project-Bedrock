// Rider movement resolution against one sub-level's local collision boxes:
// penetration recovery, a continuous sphere-group sweep with sliding, the
// vanilla-sized step path, and support settling.
// Migrated from TreePhysics src/physics/obb/internal/MountCollision.ts.
...
export interface MountSupport {
  readonly block?: SubLevelBlock;
  readonly localPoint: Vector3;
  /** World-space normal of the supporting sub-level face. */
  readonly normal: Vector3;
...
/**
 * Resolves one tick of rider-relative movement in the owning sub-level's
 * current local frame. Carrier displacement is deliberately excluded by the
 * caller, so moving the sub-level cannot make it collide with its own rider.
 */
export function resolveMountCollision(
  ...
  const localFeet = subLevel.body.worldPointToLocal(feet);
  ...
  if (wasGrounded || movement.y <= COLLISION_EPSILON) {
    ...
    if (settled) {
      ...
      support = {
        ...
        worldPoint: subLevel.body.localPointToWorld(settled.hit.localPoint)
      };
    }
  }
