// World-block classification for the physics pipeline: fluid detection and fluid
// surfaces, world collision-shape resolution, block material ids, contact
// normalization, and the sensor predicate. Migrated from TreePhysics
// src/physics/world/BlockClassification.ts.
...
// These physics-side literals intentionally mirror the ids declared by
// content/block_placement/SubLevelInteractionTargetBlock.ts; the physics layer
// recognizes blocks by id only and never imports gameplay modules.
const INTERACTION_TARGET_BLOCK_TYPE_ID = "sable:interaction_target";
const INTERACTION_TARGET_WATER_DEPTH_STATE = "sable:water_depth";
const INTERACTION_TARGET_WATER_KIND_STATE = "sable:water_kind";
...
function getInteractionTargetWaterState(
  ...
  const waterKind = states?.[INTERACTION_TARGET_WATER_KIND_STATE];
  const depth = states?.[INTERACTION_TARGET_WATER_DEPTH_STATE];
  if (waterKind !== 1 && waterKind !== 2) {
    throw new Error(`Waterlogged interaction target has invalid water kind: ${String(waterKind)}.`);
  }
  ...
  return {
    depth: Number(depth),
    typeId: waterKind === 1 ? "minecraft:water" : "minecraft:flowing_water"
  };
}
...
function normalizeResolvedWorldBlockCollisionShape(
  ...
  if (shapeKind === "none") return "none";
  if (shapeKind === "full") return "full";
