// Cannon-backed physics pipeline: body creation and handles, performance-tier
// substeps, contact materials, buoyancy and fluid entry, the world block mesh
// cache with its audit coordinator, and indexed world-sensor sweeps. Migrated
// from TreePhysics src/physics/simulation/CannonKernel.ts; the render-entity
// transform branch is dropped (sable publishes poses through its own renderer).
...
import { AabbSAPBroadphase } from "./AabbSAPBroadphase.js";
import { BoxNarrowphase } from "./BoxNarrowphase.js";
import type {
  PhysicsBodyAabb,
...
import { quaternionToContinuousEulerDegreesYzx } from "../../../api/math/RotationContinuity.js";
import { getIndexedWorldSensorCandidateChunkKeys } from "./world/sensor/WorldSensorChunkIndex.js";
import {
  EPSILON_1E8,
  isFiniteVector as isFiniteVector3
} from "../../../util/SableVector3Utils.js";
import { safeGetBlock } from "../../../util/LevelAccelerator.js";
import { type GreedyBox } from "../../../api/physics/collider/GreedyBoxMesher.js";
import {
  RADIANS_TO_DEGREES,
  compareStrings,
...
} from "./CannonColliderNormalization.js";
import {
  ...
  type FluidSurface
} from "../../chunk/WorldBlockClassification.js";
import {
...
} from "./world/WorldMeshCache.js";
import {
  createIndexedWorldSensorSweep,
  findIndexedWorldSensorContact,
  getIndexedWorldSensorShapes,
  integerBoundsOverlapGreedyBox
} from "./world/sensor/WorldSensorScan.js";
import {
  CannonPhysicsPipelineAfterEvents,
  type CannonPipelineCollisionAfterEvent,
  type CannonPipelineIndexedWorldSensorHit
} from "./CannonPhysicsPipelineEvents.js";
// The physics system consumes these event payload types through the pipeline module.
export type {
...
export interface CannonPhysicsPipelineOptions {
...
export class CannonPhysicsBody {
  readonly id: number;
  readonly name?: string;
  readonly #body: Body;
  readonly #pipeline: CannonPhysicsPipeline;

  constructor(id: number, pipeline: CannonPhysicsPipeline, body: Body, name?: string) {
    this.id = id;
  ...
  get isSleeping(): boolean {
    ...
    return this.#body.sleepState === Body.SLEEPING
      && !this.#pipeline.isBodyWorldMeshWaiting(this.id);
  }
  ...
  get isInNativeFlowingFluid(): boolean {
    return this.#pipeline.getBodyIsInNativeFlowingFluid(this.id);
  }
  ...
  get isValid(): boolean {
    return this.#pipeline.hasBody(this.id);
  }

  get location(): Vector3 {
    return this.#pipeline.getBodyLocation(this.id);
  }
  ...
  getAabb(): PhysicsBodyAabb {
    const record = this.#pipeline.getBodyRecord(this.id);
  ...
  getCenterOfMass(): Vector3 {
    return this.#pipeline.getBodyLocalCenterOfMass(this.id);
  }
  ...
  get lavaSubmersionRatio(): number {
    return this.#pipeline.getBodyLavaSubmersionRatio(this.id);
  }
  ...
  applyForceAt(
    ...
    if (coordinateSpace === "local") {
      const centerOfMass = this.#pipeline.getBodyLocalCenterOfMass(this.id);
  ...
  setBuoyancyPoints(points: readonly PhysicsBodyBuoyancyPoint[]): void {
    this.#pipeline.setBodyBuoyancyPoints(this.id, points);
  }

  setCollider(collider: PhysicsBodyCollider): void {
    this.#pipeline.setBodyCollider(this.id, collider);
  }

  setColliderIncrementally(collider: PhysicsBodyCollider): void {
    this.#pipeline.setBodyColliderIncrementally(this.id, collider);
  }

  setEnvironmentCollider(collider: PhysicsBodyCollider): void {
    this.#pipeline.setBodyEnvironmentCollider(this.id, collider);
  }
  ...
  setCenterOfMass(centerOfMass: Vector3): void {
    ...
    this.#pipeline.updateBodyShapeOffset(this.id);
    this.#body.aabbNeedsUpdate = true;
    this.wakeUp();
  }
  ...
  remove(): void {
    this.#pipeline.removeBody(this.id);
  }
  ...
  wakeUp(): void {
    if (this.#body.type !== Body.STATIC) this.#pipeline.wakeBody(this.id);
  }
}

export class CannonPhysicsPipeline {
  readonly afterEvents = new CannonPhysicsPipelineAfterEvents();
  readonly #beforeSubstepCallbacks = new Set<() => void>();
  readonly #bodies = new Map<number, CannonPhysicsBody>();
  ...
  readonly #handlesByCannonBody = new Map<Body, CannonPhysicsBody>();
  ...
  constructor(options: CannonPhysicsPipelineOptions = {}) {
  ...
  configure(options: CannonPhysicsPipelineOptions = {}): void {
    ...
    // Reuse the SAP instance: replacing it on every quality change would keep
    // its addBody/removeBody listeners registered on the world.
  ...
  createBody(dimension: Dimension, options: PhysicsBodyOptions, requestedId?: number): CannonPhysicsBody {
    ...
    const handle = new CannonPhysicsBody(id, this, body, options.name);
  ...
  getBodies(): readonly CannonPhysicsBody[] {
  ...
  private emitCollisionAfterEvent(event: CannonPipelineCollisionAfterEvent): void {
  ...
  private emitIndexedWorldSensorCollisions(): void {
    ...
    for (const [id, body] of this.#cannonBodies) {
      ...
      let hits: CannonPipelineIndexedWorldSensorHit[] | undefined;
