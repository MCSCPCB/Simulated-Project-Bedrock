...
const ORIENTATION_ROTATIONS: Readonly<Record<string, Vector3>> = {
...
export function resolveSubLevelBlockVisualOffset(
  ...
  if (!isHeadOrSkull(typeName(typeId))) return undefined;
  const facing = firstState(states, ["facing_direction", "minecraft:facing_direction"]);
...
function isNumericDirectionState(value: boolean | number | string): boolean {
  return typeof value === "number" && Number.isInteger(value) && value >= 0 && value <= 3;
}
export function usesFrontFacingDirectionStateMapping(
  name: string,
  states: Readonly<Record<string, boolean | number | string>>
): boolean {
