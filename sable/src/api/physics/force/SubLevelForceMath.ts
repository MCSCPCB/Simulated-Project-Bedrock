// Impulse arithmetic behind the sub-level force queue: piston and explosion
// magnitudes, the per-body impulse accumulator, explosion direction and merge
// keys, world occlusion rays, and the batch clamps.
// Migrated from TreePhysics src/content/contraption/effects/Math.ts.
...
import { sphereIntersectsAabb } from "../collider/SubLevelContactQuery.js";

export const MAX_PISTON_EFFECTS_PER_SUB_LEVEL = 16;
...
const MAX_EXPLOSION_EFFECTS_PER_SUB_LEVEL = 16;
export const MAX_OCCLUSION_RAYS_PER_SUB_LEVEL = 4;
...
export function createAccumulator(body: RigidBodyHandle): BodyImpulseAccumulator {
...
export function selectNearbyExplosions(explosions: readonly PendingExplosionEffect[], bounds: PhysicsBodyAabb): PendingExplosionEffect[] {
  ...
  for (const effect of explosions) {
    ...
    if (index >= MAX_EXPLOSION_EFFECTS_PER_SUB_LEVEL) continue;
    selected.splice(index, 0, { effect, score });
    if (selected.length > MAX_EXPLOSION_EFFECTS_PER_SUB_LEVEL) selected.pop();
  }
