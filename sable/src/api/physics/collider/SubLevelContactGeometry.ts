// Geometry shared by the sub-level contact queries: per-block local collision
// boxes, the body's orthonormal basis, world-to-local bounds conversion, and
// the oriented-box against axis-aligned-box separating-axis test.
// Migrated from TreePhysics src/content/contraption/interaction/ContactGeometry.ts.
...
export function getLocalCollisionBoxes(block: SubLevelBlock, requireCollisionResponse: boolean): readonly LocalBox[] {
...
export function getSubLevelBasis(subLevel: ServerSubLevel): OrthonormalBasis {
  const origin = subLevel.body.localPointToWorld({ x: 0, y: 0, z: 0 });
  return {
    x: normalize(subtract(subLevel.body.localPointToWorld({ x: 1, y: 0, z: 0 }), origin)),
    y: normalize(subtract(subLevel.body.localPointToWorld({ x: 0, y: 1, z: 0 }), origin)),
    z: normalize(subtract(subLevel.body.localPointToWorld({ x: 0, y: 0, z: 1 }), origin))
  };
}

export function worldAabbToLocalBounds(subLevel: ServerSubLevel, bounds: PhysicsBodyAabb): PhysicsBodyAabb {
  const corners: Vector3[] = [];
  for (const x of [bounds.min.x, bounds.max.x]) {
    for (const y of [bounds.min.y, bounds.max.y]) {
      for (const z of [bounds.min.z, bounds.max.z]) corners.push(subLevel.body.worldPointToLocal({ x, y, z }));
    }
  }
...
/** Separating-axis test for an oriented sub-level block against an axis-aligned piston volume. */
