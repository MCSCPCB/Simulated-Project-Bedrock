// Minimum-box voxel meshing for sub-level colliders: covers a set of integer
// block locations with deterministic, non-overlapping boxes. Migrated from
// TreePhysics src/physics/contraption/Mesher.ts.
 ...
 * This minimum-box mesher intentionally differs from the cannon pipeline's
...
function meshVoxelsInAxisOrder(
  ...
  const remaining = new Set(locations.map(blockLocationKey));
  const sortOrder = [order[2], order[1], order[0]] as const;
  const starts = [...remaining]
    .map(parseBlockLocationKey)
    .sort((left, right) =>
  ...
  for (const start of starts) {
    if (!remaining.has(blockLocationKey(start))) continue;
    ...
    for (let y = 0; y < size.y; y++) {
      for (let z = 0; z < size.z; z++) {
        for (let x = 0; x < size.x; x++) {
          remaining.delete(blockLocationKey({ x: start.x + x, y: start.y + y, z: start.z + z }));
        }
      }
    }
  ...
  for (let y = 0; y < size.y; y++) {
    for (let z = 0; z < size.z; z++) {
      for (let x = 0; x < size.x; x++) {
        ...
        if (!remaining.has(blockLocationKey({
          x: start.x + offset.x,
          y: start.y + offset.y,
          z: start.z + offset.z
        }))) return false;
