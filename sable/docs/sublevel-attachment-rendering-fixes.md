...
- 藤蔓复用现有多面附着的射线穿透、同格位掩码合并、重复面拒绝和新增面支撑验证。支撑判断传入实际侧面，避免把泥土路径等局部模型的完整底面当作可附着侧面。
...
- 藤蔓四向放置、同格追加、重复拒绝、失去支撑后逐面移除、下方藤蔓级联、存档恢复，以及两种状态键命名形式。
- 玩家射线穿过已有幽匿脉络或藤蔓追加侧面时，只增加附着面，保留一个方块并触发一次放置音效。
...
注册表保留 16 种侧面组合，为每种 `vine.faces` 加入 `up`。顶部几何复用已验证的北侧薄片，绕槽位中心旋转 `-90°`，保持原版藤蔓向内 `0.8` 像素的位置、UV 和 `alpha_test_tint` 材质。基础绘制与乘色绘制共用状态筛选，保持完全重合的几何；没有增加染色偏移。
...
放置仅顶部附着的藤蔓后，可以同格追加侧面。新增或替换上方方块时，按其底面支撑更新顶部；移除顶部支撑时保留仍有支撑的侧面；侧面全部失效但上方仍有完整支撑底面时保留顶部；全部支撑消失时拆除。上方藤蔓可以向下延续相同侧面，但不能凭此生成水平顶部。点击下方方块的顶面仍不提供藤蔓底部附着。

藤蔓继续使用既有 sparse 与共享池，因为表面植被沿用 sparse 气候采样。没有为顶部增加独立渲染实体，仍使用独立纹理路径、foliage colormap 和 `playAnimation` 状态输入。

验证覆盖底面放置射线、重复放置与音效、顶部和侧面互转、特殊模型底面、原版状态传入战利品 API、主世界捕获和真实序列化后的重建。箱子和粉雪在放置、邻接替换、捕获与存档恢复中均不产生顶部；上方藤蔓仅延续侧面，失去源支撑时级联移除。六向碰撞面覆盖检查包含半高实体、拼接、重叠、重复面、狭缝、内部空洞和偏离接触平面的盒面。全部 32 种顶部与侧面组合检查 sparse 和共享池中的基础/染色面数量、逐顶点重合、原版顶部高度和空槽位隐藏。

此次规则修正后重新构建 BP/RP，完整回归 71 项全部通过，TypeScript 严格检查为 0 个错误，差异格式检查通过。重建前后资源包的全部 854 个文件内容与路径校验一致。原生客户端的最终显示和交互仍需游戏内实测。
