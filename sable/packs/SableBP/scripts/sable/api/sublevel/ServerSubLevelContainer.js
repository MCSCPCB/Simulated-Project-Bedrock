...
import { resolveVanillaBlockBreakSound, resolveVanillaBlockHitSound, resolveVanillaBlockPlaceSound } from "../../content/sublevel_sounds/SubLevelBlockSounds.js";
import { BLOCK_BREAK_PARTICLE_PROFILE, BLOCK_HIT_PARTICLE_PROFILE, spawnSubLevelBlockDestructParticle } from "../../content/particle/SubLevelBlockParticles.js";
...
export class ServerSubLevelContainer {
    ...
    initialize() {
        ...
        for (const id of this.#storage.listSubLevelIds()) {
            ...
            try {
                ...
                if (!saved)
                    throw new Error(`Stored sub-level manifest entry ${id} has no structure record.`);
    ...
    handleVisualEntityLoad(entity) {
        if (entity.typeId !== "sable:block" && entity.typeId !== "sable:block_carrier"
            ...
            && !entity.typeId.startsWith("sable:fancy_pool_"))
            return;
        system.run(() => {
            if (!entity.isValid || this.#interactionSystem.isVisualEntity(entity.dimension.id, entity.id))
                return;
            entity.remove();
        });
    }
    tick(currentTick) {
        ...
        for (const [id, saved] of this.#pendingRestores) {
            try {
                ...
                if (!isSubLevelRegionLoaded(dimension, saved.origin, saved.blocks))
                    continue;
        ...
        for (const record of [...this.#recordsByHandleId.values()]) {
            if (record.removed || !record.handle.isValid)
                continue;
            ...
            if (!isRecordRegionLoaded(record))
                continue;
            if (!record.renderData.hasKnownIntegrityFailure() && record.renderData.hasIntactEntities())
                continue;
    ...
    handleContainerNativeDeath(ownerId, binding) {
        ...
        if (!saved)
            throw new Error(`Native death storage ${binding.storageId} has no sub-level owner ${ownerId}.`);
    ...
    handleContainerUnexpectedRemoval(ownerId, _binding) {
        ...
        const saved = this.#storage.loadSubLevel(ownerId);
        if (!saved)
            throw new Error(`Unexpected storage removal has no sub-level owner ${ownerId}.`);
    ...
    createSubLevelFromRegion(dimension, from, to, options) {
        ...
        for (let y = minimum.y; y <= maximum.y; y++) {
            for (let z = minimum.z; z <= maximum.z; z++) {
                for (let x = minimum.x; x <= maximum.x; x++) {
                    ...
                    if (!block)
                        throw new Error(`Selected block is unavailable at ${x},${y},${z}.`);
        ...
        const removals = [...captured].sort((left, right) => (sourceRemovalOrder(left) - sourceRemovalOrder(right)
        ...
        try {
            for (const snapshot of removals) {
                ...
                if (!block?.isValid)
                    throw new Error("A selected block became unavailable during capture.");
    ...
    breakBlockForPlayerEdit(_player, itemStack, handle, block) {
        ...
        if (!record || record.removed || !handle.isValid)
            return false;
        ...
        const removedBlocks = [current, ...previousBlocks.filter(entry => (blockLocationKey(entry.localLocation) !== targetKey
        ...
        if (remaining.length > 0)
            this.#saveRecord(record, bindings, remaining);
        ...
        try {
            ...
            if (support.stateUpdates.size > 0)
                this.#applyStateUpdates(record, support.stateUpdates);
        }
        catch (error) {
        ...
        for (const [index, removedBlock] of removedBlocks.entries()) {
            ...
            spawnSubLevelBlockDestructParticle(dimension, position, removedBlock, record.subLevel.foliageTint, BLOCK_BREAK_PARTICLE_PROFILE);
    ...
    emitBlockMiningEffects(handle, block) {
        ...
        spawnSubLevelBlockDestructParticle(dimension, position, block, record.subLevel.foliageTint, BLOCK_HIT_PARTICLE_PROFILE);
    ...
    placeBlockForPlayerEdit(player, itemStack, handle, _supportBlock, placement, cardinalDirection, placementFace) {
        ...
        if (!record || record.removed || !handle.isValid)
            return false;
        if (getSubLevelBlockRegistration(itemStack.typeId)?.placeable === false)
            return false;
        if (handle.getBlockAtLocalLocation(placement))
            return false;
        const placed = buildPlacedBlock(player, itemStack.typeId, placement, cardinalDirection, placementFace);
        if (!placed)
            return false;
        const previousBlocks = [...handle.blocks];
        const previousBindings = new Set(this.#containerBindings(record).map(binding => binding.storageId));
        try {
    ...
    #captureWorldData(dimension, blocks, origin) {
        ...
        for (const block of blocks) {
            ...
            if (data !== undefined)
                worldData.set(blockLocationKey(block.localLocation), data);
        }
        return worldData;
    }
    ...
    #recreateRender(record, blocks) {
        ...
        system.run(() => {
            if (!record.removed && record.renderData === next)
                this.#containers.refreshModelStates(record.handle);
        });
    }
    ...
    #restoreSubLevel(saved) {
        const record = this.#createRuntimeRecord(saved.id, world.getDimension(saved.dimensionId), saved.origin, saved.blocks, saved.foliageTint
...
function buildPlacedBlock(_player, typeId, placement, cardinalDirection, placementFace) {
    ...
    if (stairDirectionState)
        states[stairDirectionState] = directionForCardinal(cardinalDirection);
    ...
    if (upsideDownState && placementFace)
        states[upsideDownState] = placementFace === "down";
    ...
    if (verticalHalfState && placementFace)
        states[verticalHalfState] = placementFace === "down" ? "top" : "bottom";
...
function resolvePlacedDirection(typeId, cardinalDirection, placementFace) {
    if (typeId === "minecraft:cocoa" && placementFace) {
        ...
        if (placementFace === "north")
            return 0;
        if (placementFace === "east")
            return 1;
        if (placementFace === "south")
            return 2;
